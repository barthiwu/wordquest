// Country list for the onboarding flag picker, Passport display, and
// Leaderboard Continent/Country filters. ISO 3166-1 alpha-2 codes paired
// with a display name and continent. Flags are rendered on demand from
...
export function countryForCode(countryCode: string | null | undefined): Country | null {
  ...
  return BY_CODE[countryCode.toUpperCase()] ?? null;
}

export function countryNameForCode(countryCode: string | null | undefined): string | null {
  return countryForCode(countryCode)?.name ?? null;
}
